...
WeaponAPI.EquipWeapon = function(weaponName, slot, id, hash)
    ...
    if slot == 1 then
        if #EquippedWeapons > 0 then
            local newGUID = moveInventoryItem(inventoryId, EquippedWeapons[1].guid:Buffer(), weaponItem:Buffer(), 1)
            if not newGUID then
                -- RESILIENCE FIX: previously this just failed and left the
                -- stale entry in EquippedWeapons forever, breaking every
                -- subsequent equip attempt with the same error. The entry is
                -- almost certainly a "ghost" (the game no longer recognizes
                -- that guid as valid, e.g. after a forced removal that
                -- couldn't clean this table). Drop it and continue as if the
                -- slot was empty, instead of permanently jamming.
                print("^1[fdb-weapons] Cannot move item - stale EquippedWeapons entry detected, clearing and retrying as empty slot^7")
                table.remove(EquippedWeapons, 1)
    ...
    if move then
        Citizen.InvokeNative(0x12FB95FE3D579238, playerPedId, EquippedWeapons[1].guid:Buffer(), true, 1, false, false)
    end
...
WeaponAPI.RemoveWeaponFromPeds = function(weaponName, serial)
    ...
    if isWeaponAGun and isWeaponOneHanded then
        for k, v in pairs(EquippedWeapons) do
            if v.id == serial then
                Citizen.InvokeNative(0x3E4E811480B3AE79, 1, v.guid:Buffer(), 1, joaat("REMOVE_REASON_DEFAULT"))
    ...
    if weaponRemoved and #EquippedWeapons > 0 then
        ...
        local moveSuccess = moveInventoryItem(inventoryId, EquippedWeapons[1].guid:Buffer(), weaponItem:Buffer(), 0)
        if moveSuccess then
            Citizen.InvokeNative(0x12FB95FE3D579238, playerPedId, EquippedWeapons[1].guid:Buffer(), true, 0, false, false)
        else
