const InventoryContainer = Vue.createApp({
    ...
    methods: {
        getInitialState() {
            return {
                ...
                otherInventoryHotbarOffset: null,
                otherInventoryHotbarMaxSlots: null,
        ...
        openInventory(data) {
            ...
            if (data.other) {
                ...
                this.otherInventoryHotbarOffset = data.other.hotbarOffset || null;
                this.otherInventoryHotbarMaxSlots = data.other.hotbarMaxSlots || null;
        ...
        async closeInventory() {
            ...
            if (wasTradeActive && currentTradeId) {
                axios.post("https://fdb-inventory/CancelTrade", { tradeId: currentTradeId }).catch(() => {});
            }

            Object.assign(this, this.getInitialState());
            try {
                await axios.post("https://fdb-inventory/CloseInventory", { name: inventoryName });
                if (wasHotbarEnabled) {
                    this.toggleHotbar({
                        open: true,
                        items: hotbarItems,
        ...
        async handleDropOnInventoryContainer() {
            if (this.isOtherInventoryEmpty && this.dragStartInventoryType === "player") {
                ...
                try {
                    const response = await axios.post("https://fdb-inventory/DropItem", {
                        ...newItem,
        ...
        async handlePurchase(sourceSlot, sourceItem, transferAmount, sourceInventoryType, targetSlot = null) {
            ...
            this.busy = true;
            try {
                const response = await axios.post("https://fdb-inventory/AttemptPurchase", {
                    item: sourceItem,
                    ...
                    if (typeof quantity === "string") {
                        switch (quantity) {
                            ...
                            case "all":
                                ...
                                break;
                            case "enteramount":
                                const amounttt = await axios.post("https://fdb-inventory/GiveItemAmount")
        ...
        async useItem(item) {
            ...
            if (playerItemKey) {
                try {
                    ...
                    await axios.post("https://fdb-inventory/UseItem", {
                        inventory: "player",
                        item: item,
                    });
                } catch (error) {
        ...
        async giveItem(item, quantity) {
            if (item && item.name) {
                ...
                if (playerHasItem) {
                    ...
                    try {
                        const response = await axios.post("https://fdb-inventory/GiveItem", {
                            item: selectedItem,
                            amount: amountToGive,
        ...
        async splitAndPlaceItem(item, inventoryType, splitamount = 'half') {
            ...
            if (item && item.amount > 1) {
                if (splitamount == 'half') {
                    ...
                    const inputAmount = await axios.post("https://fdb-inventory/GiveItemAmount")
        ...
        inventoryError(slot) {
            ...
            axios.post("https://fdb-inventory/PlayDropFail", {}).catch((error) => {
        ...
        /* openWeaponAttachments() {
            ...
            if (!this.showWeaponAttachments) {
                ...
                axios
                    .post("https://fdb-inventory/GetWeaponData", JSON.stringify({ weapon: this.selectedWeapon.name, ItemData: this.selectedWeapon }))
                    .then((response) => {
        ...
        removeAttachment(attachment) {
            ...
            axios
                .post("https://fdb-inventory/RemoveAttachment", JSON.stringify({ AttachmentData: attachment, WeaponData: this.selectedWeapon }))
                .then((response) => {
        ...
        postInventoryData(fromInventory, toInventory, fromSlot, toSlot, fromAmount, toAmount) {
            ...
            axios
                .post("https://fdb-inventory/SetInventoryData", {
        ...
        addItemToTrade(item, amount) {
            ...
            axios.post("https://fdb-inventory/AddTradeItem", {
        ...
        async addItemToTradeWithPrompt(item) {
            if (!this.isTradeActive || !this.tradeId) return;
            try {
                const response = await axios.post("https://fdb-inventory/GiveItemAmount");
        ...
        removeItemFromTrade(tradeSlot) {
            if (!this.isTradeActive || !this.tradeId) return;
            axios.post("https://fdb-inventory/RemoveTradeItem", {
                tradeId: this.tradeId,
                tradeSlot: tradeSlot,
            }).catch((error) => {
        ...
        confirmTrade() {
            if (!this.isTradeActive || !this.tradeId) return;
            axios.post("https://fdb-inventory/ConfirmTrade", {
                tradeId: this.tradeId,
            }).catch((error) => {
        ...
        cancelTrade() {
            if (!this.isTradeActive || !this.tradeId) return;
            axios.post("https://fdb-inventory/CancelTrade", {
                tradeId: this.tradeId,
            }).catch((error) => {
        ...
        initiateTrade(targetId) {
            axios.post("https://fdb-inventory/InitiateTrade", {
                targetId: targetId,
            }).catch((error) => {
    ...
    mounted() {
        ...
        axios.interceptors.request.use((config) => {
            if (config.url && config.url.startsWith("https://fdb-inventory/")) {
