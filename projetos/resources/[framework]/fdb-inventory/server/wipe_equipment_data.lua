...
RegisterCommand('wipe_equipment', function(source, args, rawCommand)
    ...
    print('[fdb-inventory] Wiping equipmentSlots from all players...')
    exports.oxmysql:execute(
        "UPDATE players SET metadata = JSON_REMOVE(metadata, '$.equipmentSlots')",
        {},
        function(rowsAffected)
            print('[fdb-inventory] DONE. equipmentSlots wiped from ' .. tostring(rowsAffected) .. ' players.')
            print('[fdb-inventory] Players will get a clean empty slot structure on next login.')
        end
    )
end, true)
