...
local function RuinBackpack(source, stashId, itemName)
    ...
    TriggerClientEvent('rsg-backpacks:client:forceRuinDoff', source, stashId, itemName)
end

-- Evento de dano disparado de forma segura pelo cliente
RegisterNetEvent('rsg-backpacks:server:damageBackpack', function(damageType)
    local src = source
    ...
    print(("[rsg-backpacks Debug] server damageBackpack event received. Player: %s, Stash: %s, DamageType: %s"):format(src, stashId, damageType))

    -- 2. Controle de Rate Limit por jogador
    local now = os.time()
    if lastDamageTime[src] and (now - lastDamageTime[src] < ConfigDurability.rateLimit) then
        print("[rsg-backpacks Debug] server damageBackpack rate-limited.")
        return
    end
...
RegisterNetEvent('rsg-backpacks:server:damageGroundBackpack', function(stashId, damageType)
    local src = source
    local Player = RSGCore.Functions.GetPlayer(src)
    if not Player then return end
    ...
    print(("[rsg-backpacks Debug] server damageGroundBackpack received. Player: %s, Stash: %s, UID: %s, DamageType: %s"):format(src, stashId, uid, damageType))
    ...
    if lastDamageTime[limitKey] and (now - lastDamageTime[limitKey] < ConfigDurability.rateLimit) then
        print("[rsg-backpacks Debug] server damageGroundBackpack rate-limited.")
        return
    end
    ...
    local bp = GetBackpackByUid(uid)
    if not bp then
        print(("[rsg-backpacks Debug] server damageGroundBackpack - Backpack not found in DB for UID: %s"):format(uid))
        return
    end

    local currentDurability = bp.durability or 100
    print(("[rsg-backpacks Debug] server damageGroundBackpack - Current Durability: %s"):format(currentDurability))
    if currentDurability <= 0 then return end
