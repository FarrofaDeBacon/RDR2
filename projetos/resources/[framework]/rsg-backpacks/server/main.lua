...
for itemName, _ in pairs(Config.Backpacks) do
    RSGCore.Functions.CreateUseableItem(itemName, function(source, item)
        ...
        if isSatchel then
            if Player.PlayerData.metadata.equippedSatchel then
            ...
            if Player.PlayerData.metadata.equippedBackpack then
                TriggerClientEvent('ox_lib:notify', src, {
                    title = 'Mochila',
        ...
        if bpConfig.isClothing or usedItemName == "doctor_bag" then
            ...
            Player.Functions.SetMetaData("equippedSatchel", {
                stashId = stashId,
...
RegisterNetEvent('rsg-backpacks:server:wearBackpack', function(stashId)
    ...
    -- Seta metadados indicando que o jogador está vestindo a mochila ou bolsa
    if isSatchel then
        Player.Functions.SetMetaData("equippedSatchel", {
...
local function unequipSatchel(src)
    ...
    local eqSatchel = Player.PlayerData.metadata.equippedSatchel
    ...
    if Player.Functions.AddItem(itemName, 1, nil, info) then
        ...
        -- Detacha a bolsa visualmente no cliente
        TriggerClientEvent('rsg-backpacks:client:detachSatchel', src)
...
-- UI Doff Button for Backpack (mochila de costas)
...
RegisterNetEvent('rsg-backpacks:server:unequipSatchel', function()
    unequipSatchel(source)
end)
        ...
        if src > 0 then
            ...
            if Player then
                local msg = ("Backpack: %s\nSatchel: %s"):format(
                    json.encode(Player.PlayerData.metadata.equippedBackpack),
                    json.encode(Player.PlayerData.metadata.equippedSatchel)
                )
                TriggerClientEvent('ox_lib:notify', src, {
                    title = 'Debug Mochila',
                    description = msg,
                ...
                print("equippedBackpack: " .. json.encode(Player.PlayerData.metadata.equippedBackpack, {indent = true}))
                print("equippedSatchel: " .. json.encode(Player.PlayerData.metadata.equippedSatchel, {indent = true}))
            end
        end
    end
end)
