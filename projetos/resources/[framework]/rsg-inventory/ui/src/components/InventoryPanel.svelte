...
{#if inventoryType === 'player' && satchel && satchel.isEquipped}
  <div class="satchel-inventory-bg" class:centered-satchel-bg={shouldCenterInventory}></div>

  <div class="satchel-inventory-header" class:centered-satchel-header={shouldCenterInventory}>
    <div class="header-main-row">
      ...
      <div class="current-weight">
        ...
        <p>{((satchel.items ? Object.values(satchel.items).reduce((acc, it) => acc + (it ? (it.weight * it.amount) : 0), 0) : 0) / 1000).toFixed(1)} / {(satchel.maxweight / 1000).toFixed(1)}</p>
      </div>
    </div>
    {#if satchel.durability !== undefined}
      <div class="header-sub-row" style="margin-top: 5px;">
        <div style="display: flex; flex-direction: column; width: 100%;">
          <div style="display: flex; justify-content: space-between; font-size: 8px; color: #ebdcb9; margin-bottom: 2px; font-weight: bold;">
          ...
          <div style="width: 100%; height: 5px; background: rgba(0,0,0,0.5); border-radius: 2px; overflow: hidden; border: 1px solid rgba(194,176,128,0.25);">
            <div style="width: {satchel.durability}%; height: 100%; transition: width 0.3s; background: {satchel.durability > 50 ? '#55c05a' : (satchel.durability > 20 ? '#e08b1a' : '#c93c3c')};"></div>
          </div>
        </div>
      </div>
    {/if}
  ...
  <div class="satchel-inventory" class:centered-satchel={shouldCenterInventory}>
        ...
        {#if backpack && backpack.isEquipped && unequipBackpack}
          <div style="display: flex; align-items: center; gap: 6px;">
            <div style="display: flex; flex-direction: column; width: 60px;">
              <div style="display: flex; justify-content: space-between; font-size: 8px; color: #ebdcb9; margin-bottom: 2px; font-family: 'Open Sans', sans-serif; font-weight: bold; text-shadow: 1px 1px 1px black;">
                <span>Mochila</span>
        ...
        {#if satchel && satchel.isEquipped && unequipSatchel}
          <div style="display: flex; align-items: center; gap: 6px;">
            <div style="display: flex; flex-direction: column; width: 60px;">
              <div style="display: flex; justify-content: space-between; font-size: 8px; color: #ebdcb9; margin-bottom: 2px; font-family: 'Open Sans', sans-serif; font-weight: bold; text-shadow: 1px 1px 1px black;">
                <span>Bolsa</span>
                <span>{satchel.durability || 100}%</span>
              </div>
              <div style="width: 100%; height: 5px; background: rgba(0,0,0,0.5); border-radius: 2px; overflow: hidden; border: 1px solid rgba(194,176,128,0.25);">
                <div style="width: {satchel.durability || 100}%; height: 100%; transition: width 0.3s; background: {(satchel.durability || 100) > 50 ? '#55c05a' : ((satchel.durability || 100) > 20 ? '#e08b1a' : '#c93c3c')};"></div>
              </div>
            </div>
            <button class="equipped-backpack-header-btn" onclick={unequipSatchel} title="Guardar Bolsa nos Bolsos" style="margin-left: 0;">
