...
RegisterNetEvent('rsg-inventory:client:openInventory', function(items, other)
    CreateThread(function()
        ...
        local eqSatchel = PlayerData.metadata and PlayerData.metadata.equippedSatchel

        if tempBackpackUid and tempBackpackModel then
            -- Caso de abrir baú do chão (target)
            backpackData = lib.callback.await('rsg-inventory:server:getBackpackStash', tempBackpackUid, tempBackpackModel)
            if backpackData then
                backpackData.autoOpen = autoOpenBackpack
                backpackData.isEquipped = false
            end
        else
            -- Inventário próprio do jogador
            if eqBackpack and eqBackpack.stashId then
                ...
                local data = lib.callback.await('rsg-inventory:server:getBackpackStash', eqBackpack.stashId, model)
                if data then
                    data.isEquipped = true
                    data.slots = 0 -- Oculta slots para a mochila de costas
                    data.autoOpen = false
                    backpackData = data
                end
            end
            ...
            if eqSatchel and eqSatchel.stashId then
                local model = "p_cs_satchel01x"
                if eqSatchel.itemName == "doctor_bag" then model = "p_bag01x" end
                
                local data = lib.callback.await('rsg-inventory:server:getBackpackStash', eqSatchel.stashId, model)
                if data then
                    data.isEquipped = true
                    data.autoOpen = autoOpenBackpack
                    satchelData = data
                end
            end
        end
