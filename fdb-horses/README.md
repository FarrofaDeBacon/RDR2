...
**fdb-horses** is a comprehensive horse management system for RedM servers using the RSG Framework. It provides full ownership, persistent data, customization, XP progression, bonding mechanics, and interactive horse behaviors.
...
1. Place `fdb-horses` into `resources/[rsg]/`
2. Ensure folder name is exactly `fdb-horses`

### Step 2: Database Setup
```bash
# Import SQL file
mysql -u username -p database_name < installation/fdb-horses.sql
```
...
#### `fdb-horses:server:SaveComponents`
**Security:**
...
```mermaid
sequenceDiagram
    Player A->>Server: TriggerEvent('fdb-horses:server:TradeHorse', playerB_id, horse_id)
...
TriggerServerEvent('fdb-horses:server:TradeHorse', targetPlayerId, horseId)
...
if IsEntityDead(horsePed) and not IsBeingRevived then
    ...
    if IsEntityDead(horsePed) then
        TriggerServerEvent('fdb-horses:server:HorseDied', horseid, horsename)
    end
end
```
...
```lua
local level = exports['fdb-horses']:CheckHorseLevel()
...
```lua
local bonding = exports['fdb-horses']:CheckHorseBondingLevel()
...
```lua
local horsePed = exports['fdb-horses']:CheckActiveHorse()
if horsePed ~= 0 then
...
#### `fdb-horses:server:GetAllHorses`
...
#### `fdb-horses:server:GetActiveHorse`
...
#### `fdb-horses:server:CheckComponents`
...
#### `fdb-horses:server:GetHorse`
```lua
local horses = lib.callback.await('fdb-horses:server:GetHorse', false, 'valentine')
```
...
-- Verify horse exists
local horse = exports['fdb-horses']:CheckActiveHorse()
...
- **Issues:** [GitHub Issues](https://github.com/Rexshack-RedM/fdb-horses/issues)
