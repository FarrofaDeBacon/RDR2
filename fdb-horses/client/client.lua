    ...
    pcall(function()
        exports.ox_target:addLocalEntity(horsePed, {
            ...
            },
            {
                ...
                distance = 2.5,
                onSelect = function()
                    TriggerEvent('fdb-horses:client:inventoryHorse')
                end,
            },
        })
    end)
...
local function Flee()
    ...
    if Config.StoreFleedHorse then
        ...
        TriggerServerEvent('fdb-horses:server:fleeStoreHorse', closestStable)
    end
    RemoveHorseTarget()
...
local DisableCamera = function()
    ...
    for k, v in pairs(entities) do
        ...
        TriggerServerEvent('fdb-horses:server:SetPlayerBucket', false, v.ped)
...
RegisterNetEvent('fdb-horses:client:custShop', function(data)
    if (horsePed == 0) then
...
RegisterCommand('sethorsename', function()
    ...
    TriggerServerEvent('fdb-horses:renameHorse', input[1])
end, false)
        ...
        options = {
            {
                ...
                event = 'fdb-horses:client:movehorse',
            ...
            },
            {
                ...
                event = 'fdb-horses:client:MenuDel',
                args = { stableid = stableid },
                arrow = true
            },
            {
            ...
            },
            {
                ...
                event = 'fdb-horses:client:storehorse',
                args = { stableid = stableid },
                arrow = true
            },
        }
    ...
    RSGCore.Functions.TriggerCallback('fdb-horses:server:GetActiveHorse', function(data, newnames)
        if horsePed ~= 0 then
            ...
            if player ~= -1 and distance < 1.5 then
                ...
                TriggerServerEvent('fdb-horses:server:TradeHorse', playerId, horseId)
    ...
    RSGCore.Functions.TriggerCallback('fdb-horses:server:GetActiveHorse', function(data)
        if (data) then
            ...
            if (location) then
                ...
                local horseMetadata = nil
                if data.metadata and data.metadata ~= "" then
                    horseMetadata = json.decode(data.metadata)
                end
                TriggerEvent('fdb-horses:client:ApplySurvivalData', horsePed, horseMetadata)
    ...
    MenuData.Open('default', GetCurrentResourceName(), 'main_character_creator_menu',
        {
            ...
            elseif data.current.value == 'buy' then
                TriggerServerEvent('fdb-horses:server:SaveComponents', horseComps[horsedata.horseid], horsedata.horseid)
                DisableCamera()
...
RegisterNetEvent('fdb-horses:client:SpawnHorse', function(data)
    ...
    TriggerServerEvent("fdb-horses:server:SetHoresActive", data.player.id)
    lib.notify({ title = locale('cl_success_title'), description = locale('cl_success_horse_active'), type = 'success', duration = 7000 })
end)

AddEventHandler('fdb-horses:client:FleeHorse', function()
    if horsePed then
        getControlOfEntity(horsePed)
...
RegisterNetEvent('fdb-horses:client:storehorse', function(data)
    if (horsePed ~= 0) then
        TriggerServerEvent('fdb-horses:server:SetHoresUnActive', HorseId, data.stableid)
    ...
    local menu = {
        {
            ...
            icon = 'horse',
            event = 'fdb-horses:client:SpawnHorse',
            args = { player = data },
            arrow = true
        },
        {
            ...
            event = 'fdb-horses:client:custShop',
            args = { player = data },
            arrow = true
        },
    }
...
-- sell horse menu
RegisterNetEvent('fdb-horses:client:MenuDel', function(data)
        ...
        options[#options + 1] = {
            ...
            onSelect = function()
                ...
                if alert == 'confirm' then
                    TriggerServerEvent('fdb-horses:server:deletehorse', { horseid = v.id })
                end
            end
        }
    end
...
-- move horse menu
------------------------------------
RegisterNetEvent('fdb-horses:client:movehorse', function(data)
    local horses = lib.callback.await('fdb-horses:server:GetHorse', false, data.stableid)
    ...
    for _, stableConfig in pairs(Config.StableSettings) do
        if stableConfig.stableid ~= currentStableId then
            ...
            options[#options + 1] = {
                ...
                arrow = false,
                onSelect = function()
                    TriggerServerEvent('fdb-horses:server:MoveHorse', horseId, stableConfig.stableid)
                    lib.showContext('stable_menu')
                end
            }
        end
...
-- horse inventory
------------------------------------
RegisterNetEvent('fdb-horses:client:inventoryHorse', function()
    ...
    RSGCore.Functions.TriggerCallback('fdb-horses:server:GetActiveHorse', function(data)
        ...
        TriggerServerEvent('fdb-horses:server:openhorseinventory', data.horseid)
    end)
end)
...
-- player feed horse
------------------------------------
RegisterNetEvent('fdb-horses:client:playerfeedhorse')
AddEventHandler('fdb-horses:client:playerfeedhorse', function(itemName)
    local pcoords = GetEntityCoords(cache.ped)
    local hcoords = GetEntityCoords(horsePed)
    ...
    if Config.HorseFeed[itemName] ~= nil then
        local feedData = Config.HorseFeed[itemName]
        
        if feedData["ismedicine"] == true then
            -- IS MEDICINE
            Citizen.InvokeNative(0xCD181A959CFDD7F4, cache.ped, horsePed, -1355254781, 0, 0) -- TaskAnimalInteraction
            local medicineHash = feedData["medicineHash"] or "consumable_horse_stimulant"
            TaskAnimalInteraction(cache.ped, horsePed, -1355254781, GetHashKey(medicineHash), 0)

            local valueHealth = Citizen.InvokeNative(0x36731AC041289BB1, horsePed, 0) or 0
            local valueStamina = Citizen.InvokeNative(0x36731AC041289BB1, horsePed, 1) or 0
            
            Citizen.Wait(3500)
            
            -- Apply native health/stamina if config has it > 0
            if feedData["health"] and feedData["health"] > 0 then
                Citizen.InvokeNative(0xC6258F41D86676E0, horsePed, 0, valueHealth + feedData["health"])
            end
            if feedData["stamina"] and feedData["stamina"] > 0 then
                Citizen.InvokeNative(0xC6258F41D86676E0, horsePed, 1, valueStamina + feedData["stamina"])
            end
            ...
            if feedData.cure_illness then TriggerEvent('fdb-horses:client:Cure', 'illness') end
            if feedData.cure_poison then TriggerEvent('fdb-horses:client:Cure', 'poison') end
            ...
            if feedData.hunger and feedData.hunger > 0 then
                TriggerEvent('fdb-horses:client:Feed', feedData.hunger)
            end
            if feedData.thirst and feedData.thirst > 0 then
                TriggerEvent('fdb-horses:client:Drink', feedData.thirst)
            end
            
            -- Optionally fill cores slightly
            if feedData.health and feedData.health > 0 then
                local h = Citizen.InvokeNative(0x36731AC041289BB1, horsePed, 0) or 0
                Citizen.InvokeNative(0xC6258F41D86676E0, horsePed, 0, h + feedData.health)
            end
            if feedData.stamina and feedData.stamina > 0 then
                local s = Citizen.InvokeNative(0x36731AC041289BB1, horsePed, 1) or 0
                Citizen.InvokeNative(0xC6258F41D86676E0, horsePed, 1, s + feedData.stamina)
            end
            ...
            RSGCore.Functions.TriggerCallback('fdb-horses:server:GetActiveHorse', function(data)
                if data then
                    lib.notify({
                        ...
                        description = string.format(locale('cl_horse_fed_desc'), data.name, feedData.hunger or 0, feedData.thirst or 0),
                        type = 'success',
                        duration = 5000
                    })
                end
...
-- player brush horse
------------------------------------
RegisterNetEvent('fdb-horses:client:playerbrushhorse')
AddEventHandler('fdb-horses:client:playerbrushhorse', function(itemName)
    local pcoords = GetEntityCoords(cache.ped)
    local hcoords = GetEntityCoords(horsePed)
...
-- player revive horse
------------------------------------
RegisterNetEvent("fdb-horses:client:revivehorse")
AddEventHandler("fdb-horses:client:revivehorse", function(item, data)
    ...
    while true do
        ...
        if horsePed ~= 0 then
            ...
            if horsedirt ~= lastDirtValue then
                ...
                TriggerServerEvent('fdb-horses:server:sethorseAttributes', horsedirt)
            end
        end
    end
end)
...
-- shop store
------------------------------------
RegisterNetEvent('fdb-horses:client:OpenHorseShop')
AddEventHandler('fdb-horses:client:OpenHorseShop', function()
    TriggerServerEvent('fdb-horses:server:openShop')
end)
