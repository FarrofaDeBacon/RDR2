...
RegisterNetEvent('fdb-consume:prop:cigar')
AddEventHandler('fdb-consume:prop:cigar', function(maxUses)
    ...
    if not IsEntityPlayingAnim(ped, dict, anim, 3) then
        ...
        Wait(100)
        AttachEntityToEntity(prop, ped, boneIndex, 0.015, -0.01, 0.0, 0.0, 90.0, 0.0, true, true, false, true, 1, true, false, false)
        ...
        while smoking do
            if IsEntityPlayingAnim(ped, dict, anim, 3) then
                ...
                DisableControlAction(0, 0xCC1075A7, true) -- Roda Mouse
                DisableControlAction(0, 0x295175BF, true) -- Roda Mouse

                if IsDisabledControlJustReleased(0, Config.Prompts.SmokeKey) then
                    puffsTaken = puffsTaken + 1
                    local playerPed = PlayerPedId()
                    local coords = GetEntityCoords(playerPed)
                    TriggerServerEvent("fdb-consume:server:addStress", -2)
                    Wait(1500) -- Intervalo entre os tragos para não spammar
                    if puffsTaken >= maxUses then
                        PromptSetEnabled(PropPrompt, false)
                        PromptSetVisible(PropPrompt, false)
                        PromptSetEnabled(SmokePrompt, false)
                        PromptSetVisible(SmokePrompt, false)
                        proppromptdisplayed = false
                        smoking = false
                        ClearPedSecondaryTask(ped)
                        DeleteObject(prop)
