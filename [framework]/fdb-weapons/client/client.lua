...
------------------------------------------
RegisterNetEvent('fdb-weapons:client:forceRemoveWeapon', function(serial)
    if not serial then return end

    local weaponName = nil
    if UsedWeapons[serial] then
        weaponName = UsedWeapons[serial].name
    end

    if not weaponName then
        local equipped = exports['fdb-weapons']:EquippedWeapons() or {}
    ...
    if weaponName then
        ...
        -- Failsafe: we don't know the exact weapon, but the server is
        -- telling us this serial is no longer valid. If the player is
        -- currently holding any weapon, strip it rather than risk leaving
        -- an unauthorized weapon in hand.
        local currentHash = GetSelectedPedWeapon(cache.ped)
        if currentHash and currentHash ~= joaat('weapon_unarmed') then
            RemoveWeaponFromPed(cache.ped, currentHash)
        end
    end

    UsedWeapons[serial] = nil
    if currentWeaponSerial == serial then
        currentWeaponSerial = nil
    end

    if Config.Debug then
        print('^1[fdb-weapons] Remoção forçada da arma (serial: '..tostring(serial)..') — validação de posse falhou ou item saiu do inventário^7')
    end
end)
...
RegisterNetEvent('RSGCore:Client:OnPlayerLoaded', function()
    ...
    CreateThread(function()
        ...
        Wait(5000)
        RSGCore.Functions.TriggerCallback('fdb-weapons:server:getEquippedWeapons', function(equippedWeapons)
            if equippedWeapons and next(equippedWeapons) ~= nil then
                for serial, _ in pairs(equippedWeapons) do
                    RSGCore.Functions.TriggerCallback('fdb-weapons:server:getWeaponBySerial', function(weaponData)
        ...
        RSGCore.Functions.TriggerCallback('fdb-weapons:server:getEquippedKnives', function(equippedKnives)
