...
RegisterNetEvent('rsg-inventory:server:addTradeItem', function(tradeId, item, amount)
    ...
    -- SECURITY FIX: previously a weapon put up for trade stayed in the
    -- player's hand even after leaving the inventory into escrow, since
    -- nothing here called Inventory.CheckWeapon (unlike the move/drop paths).
    if invItem.type == 'weapon' then
        Inventory.CheckWeapon(src, invItem)
    end
